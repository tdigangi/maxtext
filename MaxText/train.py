...
def train_step(model, config, state_mesh_shardings, state, data, dropout_rng):
  ...
  if config.gradient_accumulation_steps > 1:
    ...
    elif config.optimizer_memory_host_offload:
  ...
  if config.optimizer_compute_host_offload:
    ...
    state = state.replace(params = jax.device_put(state.params, jax.tree_util.tree_map(lambda x: x.with_memory_kind(kind='device'), state_mesh_shardings.params)),
                          opt_state = jax.device_put(state.opt_state, jax.tree_util.tree_map(lambda x: x.with_memory_kind(kind='device'), state_mesh_shardings.opt_state)))
    new_state = state.apply_gradients(grads=grads)
  else:
    new_state = state.apply_gradients(grads=grads)
  if config.optimizer_memory_host_offload:
    new_state = new_state.replace(params = jax.device_put(state.params, jax.tree_util.tree_map(lambda x: x.with_memory_kind(kind='pinned_host'), state_mesh_shardings.params)),
                          opt_state = jax.device_put(state.opt_state, jax.tree_util.tree_map(lambda x: x.with_memory_kind(kind='pinned_host'), state_mesh_shardings.opt_state)))
